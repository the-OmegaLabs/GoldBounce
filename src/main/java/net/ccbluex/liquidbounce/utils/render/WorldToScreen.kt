...
object WorldToScreen : MinecraftInstance() {
    ...
    fun getMatrix(matrix: Int): Matrix4f {
        val floatBuffer = BufferUtils.createFloatBuffer(16).apply {
            clear()
            val error = GL11.glGetError()
            if (error != GL11.GL_NO_ERROR) {
                chat(
                    "OpenGL error (${GL11.glGetError()}) when getting matrix: 0x${
                        matrix.toString(
                            16
                        )
                    }"
                )
                return Matrix4f() // 返回单位矩阵防止崩溃
            }

        }
        return Matrix4f().apply {
            load(floatBuffer)
            require(floatBuffer.remaining() == 0) { "Buffer underflow detected" }
        }
    }
        ...
        fun worldToScreen(
            ...
            val clipSpacePos =
                Vector4f(pointInWorld.x, pointInWorld.y, pointInWorld.z, 1f) * viewMatrix * projectionMatrix

            val ndcSpacePos =
                Vector3f(clipSpacePos.x, clipSpacePos.y, clipSpacePos.z).scale(1 / clipSpacePos.w) as Vector3f
