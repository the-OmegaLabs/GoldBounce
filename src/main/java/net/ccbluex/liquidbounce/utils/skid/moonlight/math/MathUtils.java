...
public class MathUtils {
    ...
    public static Vec3 closestPointOnFace(AxisAlignedBB aabb, EnumFacing face, double x, double y, double z) {
        ...
        switch (face) {
            case DOWN:
            case UP:
                closestX = (float) Math.max(aabb.minX, Math.min(x, aabb.maxX));
                closestY = (float) (face == EnumFacing.DOWN ? aabb.minY : aabb.maxY);
                closestZ = (float) Math.max(aabb.minZ, Math.min(z, aabb.maxZ));
                break;
            case NORTH:
            case SOUTH:
                closestX = (float) Math.max(aabb.minX, Math.min(x, aabb.maxX));
                closestY = (float) Math.max(aabb.minY, Math.min(y, aabb.maxY));
                closestZ = (float) (face == EnumFacing.NORTH ? aabb.minZ : aabb.maxZ);
                break;
            case WEST:
            case EAST:
                closestX = (float) (face == EnumFacing.WEST ? aabb.minX : aabb.maxX);
                closestY = (float) Math.max(aabb.minY, Math.min(y, aabb.maxY));
                closestZ = (float) Math.max(aabb.minZ, Math.min(z, aabb.maxZ));
                break;
            default:
