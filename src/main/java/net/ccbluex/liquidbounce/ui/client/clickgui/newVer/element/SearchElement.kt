...
class SearchElement(val xPos: Float, val yPos: Float, val width: Float, val height: Float) {
    ...
    fun drawBox(mouseX: Int, mouseY: Int, accentColor: Color): Boolean {
        ...
        if (searchBox.isFocused()) {
            RenderUtils.drawRoundedRect(xPos, yPos + height - 1F, xPos + width, yPos + height, accentColor.rgb,0.0F)
        ...
        RenderUtils.drawImage(IconManager.search, (xPos + width - 15).toInt(), (yPos + 5).toInt(), 10, 10)
