...
public open class ColorSpec2021 : ColorSpec {
    ...
    override fun getTone(
        ...
        val toneDeltaPair: ToneDeltaPair? =
            if (color.toneDeltaPair == null) null else color.toneDeltaPair?.invoke(scheme)
        ...
        if (toneDeltaPair != null) {
            ...
            if (color.background != null &&
                ...
                val bg: DynamicColor? = color.background?.invoke(scheme)
                val nContrastCurve: ContrastCurve? = nearer.contrastCurve?.invoke(scheme)
                val fContrastCurve: ContrastCurve? = farther.contrastCurve?.invoke(scheme)
            ...
            val bgTone = color?.background?.invoke(scheme)?.getTone(scheme) ?: 0.0
            val desiredRatio = color?.contrastCurve?.invoke(scheme)?.get(scheme.contrastLevel) ?: 0.0
            ...
            val bgTone1 = color?.background?.invoke(scheme)?.getTone(scheme) ?: 0.0
